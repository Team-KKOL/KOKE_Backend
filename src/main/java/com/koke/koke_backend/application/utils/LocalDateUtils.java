...
import static com.koke.koke_backend.common.singleton.Constant.FORMAT_LOCAL_DATE;
...
public class LocalDateUtils {
    ...
    public static LocalDate calculateFirstDeliveryDt(LocalDate source) {
        switch(source.getDayOfWeek()) {
            case SATURDAY -> {
                return source.plusDays(3);
            }
            case SUNDAY -> {
                return source.plusDays(2);
            }
            default -> {
                return source.plusDays(1);
            }
        }
    }
}
